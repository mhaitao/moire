...
<div class="min-h-screen bg-white text-[0.95rem] {config.theme}">
  <div class="max-w-2xl mx-auto px-5 md:px-12 mb-6">
    ...
    {#if memoList.activeMemo}
      {@const memo = memoList.activeMemo}
      <article class="py-2" id={memo.slug} in:slide>
        <button
          class="mb-6 text-[0.8rem] text-gray-400 hover:text-[var(--accent-color)] transition-colors cursor-pointer"
          onclick={() => memoList.closeMemo()}
        >
          &larr; Back
        </button>
    ...
    {:else}
      <div class="divide-y divide-gray-100">
        {#each memoList.visibleMemos as memo (memo.slug)}
          <a
            href={`#${memo.slug}`}
            class="group flex items-baseline justify-between gap-4 py-4 no-underline"
            in:slide
            onclick={(e) => {
              e.preventDefault();
              memoList.openMemo(memo.slug);
            }}
          >
            <span class="truncate text-gray-800 group-hover:text-[var(--accent-color)] transition-colors">
              {memoList.getTitle(memo)}
            </span>
            <span class="shrink-0 text-[0.75rem] text-gray-400">{format(memo.date, 'MMM d, yyyy')}</span>
          </a>
        {/each}
      </div>
